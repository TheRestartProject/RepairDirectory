...
class CompaniesHouse extends Command
{
     ...
     * @var string
     */
    protected $signature = 'companieshouse:compare {CSV file}';
     ...
     * @var string
     */
    protected $description = 'Compare database vs Companies House CSV';
    ...
    {
        // Get the list of SIC codes which indicate repair.
        $siccodes = [];

        foreach (self::REPAIR_KEYWORDS as $keyword) {
            foreach (self::SIC_CODES as $code => $desc) {
        ...
        $fn = $this->argument('CSV file');

        $handle = fopen($fn, "r");
        $postcodematches = fopen('/tmp/postcodematches.csv', 'w');
        $header = true;

        // First check is whether we can match the current businesses against Companies House data.
        ...
        // This file is big, so use fgetcsv which doesn't load it all in.
        while ($csvLine = fgetcsv($handle, 10000, ",")) {
            if ($header) {
                ...
                // Format is:
                //
                // CompanyName, CompanyNumber,RegAddress.CareOf,RegAddress.POBox,RegAddress.AddressLine1, RegAddress.AddressLine2,RegAddress.PostTown,RegAddress.County,RegAddress.Country,RegAddress.PostCode,CompanyCategory,CompanyStatus,CountryOfOrigin,Dis
                //solutionDate,IncorporationDate,Accounts.AccountRefDay,Accounts.AccountRefMonth,Accounts.NextDueDate,Accounts.LastMadeUpDate,Accounts.AccountCategory,Returns.NextDueDate,Returns.LastMadeUpDate,Mortgages.NumMortCharges,Mortgages.NumMortOut
                //standing,Mortgages.NumMortPartSatisfied,Mortgages.NumMortSatisfied,SICCode.SicText_1,SICCode.SicText_2,SICCode.SicText_3,SICCode.SicText_4,LimitedPartnerships.NumGenPartners,LimitedPartnerships.NumLimPartners,URI,PreviousName_1.CONDATE,
                //PreviousName_1.CompanyName, PreviousName_2.CONDATE, PreviousName_2.CompanyName,PreviousName_3.CONDATE, PreviousName_3.CompanyName,PreviousName_4.CONDATE, PreviousName_4.CompanyName,PreviousName_5.CONDATE, PreviousName_5.CompanyName,Previ
                //ousName_6.CONDATE, PreviousName_6.CompanyName,PreviousName_7.CONDATE, PreviousName_7.CompanyName,PreviousName_8.CONDATE, PreviousName_8.CompanyName,PreviousName_9.CONDATE, PreviousName_9.CompanyName,PreviousName_10.CONDATE, PreviousName_
                //10.CompanyName,ConfStmtNextDueDate, ConfStmtLastMadeUpDate
                $name = $csvLine[0];
                $address1 = $csvLine[4];
                $address2 = $csvLine[5];
                $town = $csvLine[6];
                $postcode = $csvLine[9];
                $sic = $csvLine[26] . $csvLine[27] . $csvLine[28] . $csvLine[29];

                $rdaddress = "$address1, $address2, $town";

                foreach ($businesses as $business) {
                    // We only need to check businesses at the same postcode.
                    if ($this->canon($postcode) == $this->canon($business->getPostcode())) {
                        // Write this to a tmp file.  This allows us to generate a shorter list of businesses in
                        // relevant postcodes so that we can refine our comparison without processing the whole
                        // caboodle, by re-running this job on that output file.
                        fputcsv($postcodematches, $csvLine);

                        // Compare the business name for an approx match.
                        $damerauLevenshtein = new DamerauLevenshtein(
                            $this->canonName(trim($name)),
                            $this->canonName(trim($business->getName()))
                        );

                        $dist = $damerauLevenshtein->getRelativeDistance();

                        if ($dist >= self::NAME_CONFIDENCE) {
                            $this->info("$name $postcode name + postcode match");
                            $matched[$business->getUid()] = $business->getName();
                        } else {
                            // Not all business names match - companies trade with different names.  But if we have
                            // an address match and a SIC Code indicating repair, then that is fairly good.
                            // Compare the business name for an approx match.
                            if (strpos(trim($address1), strtoupper($business->getAddress())) === 0) {
                                $repairBusiness = FALSE;

                                foreach ($siccodes as $code) {
                                    if (strpos($sic, '' . $code) !== FALSE) {
                                        $repairBusiness = TRUE;
                                        break;
                                    }
                                }
                                ...
                                if ($repairBusiness) {
                                    $this->info("$name $address1 $postcode repair business at same address as " . $business->getName());
                                    $matched[$business->getUid()] = $business->getName();
                                }
                            }
                        }
        ...
        foreach ($businesses as $business) {
            if (!array_key_exists($business->getUid(), $matched)) {
                $this->info(
                    "  " . $business->getName() . " - " . $business->getAddress() . " " . $business->getPostcode()
                );
            }
        }

        $this->info(
            "\nMatched " . count($matched) . " of " . count($businesses) . " published businesses in RD.\n"
        );
    }
    ...
    function canonName($str)
    {
        ...
        $str = str_ireplace(' LN,', ' LANE,', $str);
        $str = str_ireplace(' ST,', ' STREET,', $str);
        $str = str_ireplace(' RD,', ' ROAD,', $str);
        $str = str_ireplace(' LN.,', ' LANE,', $str);
        $str = str_ireplace(' ST.,', ' STREET,', $str);
        $str = str_ireplace(' RD.,', ' ROAD,', $str);

        // Remove text in brackets - this is often disambiguation text in the company name
        $str = preg_replace("/\([^)]+\)/","", $str);

        // Remove characters we don't want to compare.
        $str = $this->canon($str);

        $str = str_replace('LIMITED', '', $str);
        $str = str_replace('LTD', '', $str);
